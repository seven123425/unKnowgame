...
public class AlterProcess extends ShapeProcess {

    public AlterProcess(PageProcess control, BaseFragmentFactory fragmentFactory, BaseFragmentControl fragmentControl) {
        super(control, fragmentFactory, fragmentControl);
    }

    @Override
    protected void backLastProcess() {
        ProcessControl.getInstance().setAlterType(false);
    }
    ...
    protected String[] getPageString() {
        String[] pageString = {
                FragmentFactory.BodyTypeEnum.alter.toString()
        };
        return pageString;
    }
}
